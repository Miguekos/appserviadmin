...
export default {
  ...
  methods: {
    ...
    async input2(val) {
      console.log("ID en Input 2:", val);
      this.notif();
      this.notif = this.$q.notify({
        message: `Cliente: ${JSON.stringify(val)} seleccionado.`,
        timeout: 1000,
        color: "secondary"
      });
      this.idCliente = val;
      await this.contactoCliente(val)
        .then(() => {
      ...
      await this.direccionCliente(val)
        .then(() => {
    ...
    async onSubmit() {
      ...
      this.actualizarCotizacion({
        ...this.model,
        id_reqcot: this.id_requerimiento,
  ...
  async mounted() {
    console.log("id", this.info.id_client);
    // this.$q.loading.show();
    // await this.getClientes();
    this.id_requerimiento = this.info.id_reqcot;
    this.buscar = this.info.no_razsoc;
    this.model.date = this.info.fe_regist;
    this.model.text = this.info.de_reqcot;
    this.model.no_client = this.info.no_razsoc;

    this.input2(this.info.id_client)
      .then(resp => {
        console.log(resp);
      })
      .catch(err => {
